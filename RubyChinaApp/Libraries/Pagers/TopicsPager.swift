...
// 需要注意的是, 虽然 RubyChina 的 API 并没有使用游标方式实现, 但原理想通, 若采取游标方式实现分页, 可简化 Pager 的实现
...
class TopicsPager {
    ...
    init(withType type: RubyChinaV3.Topics.Listing.TypeFieldValue? = nil, withNodeId nodeId: String? = nil, withCustomPerPage perPage: Int = 20) {
        self.perPage = perPage
        self.currentPage = 0
        self.endpoint = RubyChinaV3.Topics.Listing(type: type, nodeId: nodeId, offset: 0, limit: self.perPage)
    }
